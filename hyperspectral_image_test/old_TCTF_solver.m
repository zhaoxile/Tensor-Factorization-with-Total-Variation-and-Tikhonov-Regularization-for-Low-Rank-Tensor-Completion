function [X,Y,TC,psnrarr,ssimarr,iter] = old_TCTF_solver(data,known,Nway,coreNway,opts,Xref)
...
%     %% adjust the rank of (X,Y)
    ...
    TC=ifft(C,[],3);
    res=norm(TC(known)-data)^2/normdata;
    TC=real(TC);
    TC(known) = data;
    if res<opts.tol          
        break;
...
%        break;
%     end
%     psnrarr(k)= TensorPSNR(TC,Xref);
%     ssimarr(k)=TensorSSIM(TC,Xref);
